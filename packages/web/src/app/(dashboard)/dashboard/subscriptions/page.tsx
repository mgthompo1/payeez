...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  ...
  trialing: { label: 'Trialing', color: 'bg-blue-500/10 text-blue-400 border-blue-500/20', icon: Clock },
  past_due: { label: 'Past Due', color: 'bg-amber-500/10 text-amber-400 border-amber-500/20', icon: AlertCircle },
  canceled: { label: 'Canceled', color: 'bg-gray-500/10 text-gray-400 border-gray-500/20', icon: XCircle },
  unpaid: { label: 'Unpaid', color: 'bg-red-500/10 text-red-400 border-red-500/20', icon: AlertCircle },
  paused: { label: 'Paused', color: 'bg-purple-500/10 text-purple-400 border-purple-500/20', icon: Pause },
  incomplete: { label: 'Incomplete', color: 'bg-gray-500/10 text-gray-400 border-gray-500/20', icon: Clock },
  incomplete_expired: { label: 'Expired', color: 'bg-gray-500/10 text-gray-400 border-gray-500/20', icon: XCircle },
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage recurring billing subscriptions</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-[#111] border border-white/10 overflow-hidden">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-sm text-gray-500">{filteredSubscriptions.length} subscriptions</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12">
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-gray-500">Create subscriptions via the API</p>
            </div>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="text-left py-3 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">MRR</th>
                  <th className="text-left py-3 px-6 text-xs font-medium text-gray-500 uppercase tracking-wider">Current Period</th>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{subscription.customers?.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      ...
                      <div className="space-y-1">
                        {subscription.subscription_items?.map((item) => (
                          <div key={item.id} className="text-sm">
                            <span className="text-white">{item.prices?.products?.name}</span>
                            {item.quantity > 1 && (
        ...
        <SheetContent className="bg-[#111] border-white/10 w-[500px] sm:max-w-[500px] overflow-y-auto">
          ...
          {selectedSubscription && (
            <div className="mt-6 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-white/5">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">{selectedSubscription.customers?.email}</div>
                  </div>
                </div>
              </div>
              ...
              {/* Items */}
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Items</h3>
              ...
              {/* Billing Period */}
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Billing Period</h3>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-white/5">
              ...
              {selectedSubscription.trial_end && (
                <div className="space-y-3">
                  <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Trial</h3>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                    <div>
                      ...
                      <code className="text-sm text-[#19d1c3]">{selectedSubscription.id}</code>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                    ...
                    >
                      {copied === 'sub_id' ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                    </Button>
                  </div>
                  <div className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                    <div>
                      <div className="text-xs text-gray-500">Customer ID</div>
                      <code className="text-sm text-gray-300">{selectedSubscription.customer_id}</code>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                      ...
                      className="h-8 w-8 text-gray-400 hover:text-white"
                    >
                      {copied === 'cus_id' ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                    </Button>
                  </div>
                </div>
              </div>
