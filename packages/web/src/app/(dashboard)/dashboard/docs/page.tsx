              ...
              {param.children?.map((child) => (
                <tr key={`${param.name}.${child.name}`} className="border-b border-white/5 bg-white/[0.02]">
                  <td className="px-4 py-2 pl-8">
                    <code className="text-cyan-400/70">↳ {child.name}</code>
                  </td>
                  <td className="px-4 py-2 text-slate-500">{child.type}</td>
                  <td className="px-4 py-2 text-slate-400">{child.description}</td>
                </tr>
              ))}
              </Fragment>
            )
...
function EndpointCard({
  ...
  return (
    <div className="rounded-xl dashboard-card border border-white/10 overflow-hidden">
      <button
      ...
      >
        ...
        <span className="text-sm text-slate-500 ml-auto hidden sm:block">{description}</span>
        {expanded ? (
...
function SectionHeader({ icon: Icon, title, description }: { icon: any; title: string; description: string }) {
  return (
    <div className="flex items-start gap-4 mb-6">
      <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#19d1c3]/20 to-emerald-400/20 flex items-center justify-center flex-shrink-0">
    ...
    <div className="p-8 space-y-8 max-w-screen-2xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-cyan-400 border-cyan-400/30">v1.0</Badge>
          <Badge variant="outline" className="text-green-400 border-green-500/30">Stable</Badge>
        </div>
      </div>

      {/* Base URL */}
      <div className="rounded-xl dashboard-card border border-white/10 p-4 flex items-center gap-4">
      ...
      <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
        <div className="space-y-4">
          <div className="rounded-xl dashboard-card border border-white/10 p-4 lg:hidden">
            <label className="text-xs uppercase tracking-[0.3em] text-slate-500">Section</label>
            <select
              value={activeTab}
              onChange={(event) => setActiveTab(event.target.value)}
              className="mt-3 w-full rounded-lg border border-white/10 bg-obsidian px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#19d1c3]/60"
            >
              {tabs.map((tab) => (
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 rounded-xl px-3 py-2 text-sm transition-all ${activeTab === tab.id
                      ? 'bg-[#19d1c3]/10 text-cyan-400 border border-cyan-400/20'
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'quickstart' && (
            <div className="space-y-6">
          <div className="rounded-2xl bg-gradient-to-br from-[#19d1c3]/10 to-emerald-400/10 border border-cyan-400/20 p-6">
            <h2 className="text-lg font-semibold text-white mb-2">Accept payments in minutes</h2>
            <p className="text-slate-400 text-sm">Follow these steps to integrate Atlas into your application. Card data never touches your servers - we handle PCI compliance for you.</p>
          </div>
          ...
          <div className="grid gap-6">
            ...
            <div className="rounded-xl dashboard-card border border-white/10 p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="h-8 w-8 rounded-lg bg-[#19d1c3]/10 flex items-center justify-center text-cyan-400 font-bold text-sm">1</div>
...
pnpm add @atlas/sdk`}
            ...
            <div className="rounded-xl dashboard-card border border-white/10 p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="h-8 w-8 rounded-lg bg-[#19d1c3]/10 flex items-center justify-center text-cyan-400 font-bold text-sm">2</div>
                <h3 className="font-semibold text-white">Create a Payment Session (Server-side)</h3>
              </div>
              <p className="text-sm text-slate-400 mb-4">
                Create a session from your backend. This returns a <code className="text-cyan-400">client_secret</code> to pass to your frontend.
              </p>
...
// Return session.id and session.client_secret to your frontend`}
            ...
            <div className="rounded-xl dashboard-card border border-white/10 p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="h-8 w-8 rounded-lg bg-emerald-400/10 flex items-center justify-center text-emerald-400 font-bold text-sm">3</div>
  ...
  res.json({ received: true });
      ...
      {activeTab === 'authentication' && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <p className="text-sm text-slate-400">
              ...
              <a href="/dashboard/api-keys" className="text-cyan-400 hover:underline">Dashboard</a>.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-4 mt-4">
              <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <code className="text-sm text-slate-400">sk_test_...</code>
                </div>
                <p className="text-xs text-slate-500">Use for development. No real charges.</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                <div className="flex items-center gap-2 mb-2">
                  <Badge className="bg-green-500/10 text-green-400 border-green-500/20">Live</Badge>
                  <code className="text-sm text-slate-400">sk_live_...</code>
                </div>
                <p className="text-xs text-slate-500">Use in production. Real charges processed.</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="p-4 rounded-lg bg-white/5 text-center">
                ...
                <div className="text-xs text-slate-500">requests/minute</div>
              </div>
              <div className="p-4 rounded-lg bg-white/5 text-center">
              ...
              <div className="p-4 rounded-lg bg-white/5 text-center">
                ...
                <div className="text-xs text-slate-500">concurrent requests</div>
              </div>
            </div>
            <p className="text-xs text-slate-500">
              Rate limit headers are included in every response: <code className="text-cyan-400">X-RateLimit-Limit</code>,
              <code className="text-cyan-400 ml-1">X-RateLimit-Remaining</code>, <code className="text-cyan-400 ml-1">X-RateLimit-Reset</code>
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6">
            <p className="text-sm text-slate-400">
              ...
              then use the <code className="text-cyan-400">client_secret</code> to mount the payment form on your frontend.
            </p>
          </div>
  ...
  "created_at": "2024-01-15T10:30:00Z"
          ...
          <EndpointCard method="GET" path="/get-session-config/:id" description="Retrieve SDK configuration (client secret)">
            <div className="space-y-4 mt-4">
              ...
              <div>
                <h4 className="text-sm font-medium text-white mb-3">Response</h4>
                <p className="text-sm text-slate-400">Returns the session configuration used by the SDK (payment methods, keys, and feature flags).</p>
              </div>
            </div>
          </EndpointCard>
        </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-lg border border-white/10 bg-white/5 p-4">
                <div className="text-xs uppercase tracking-wide text-slate-500">Purchase</div>
                <div className="text-sm text-white mt-1">Use <code className="text-cyan-400">POST /confirm-payment/:id</code> with automatic capture (default).</div>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/5 p-4">
                <div className="text-xs uppercase tracking-wide text-slate-500">Authorize</div>
                <div className="text-sm text-white mt-1">Set <code className="text-cyan-400">capture_method: "manual"</code> when creating the session.</div>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/5 p-4">
                <div className="text-xs uppercase tracking-wide text-slate-500">Capture</div>
                <div className="text-sm text-white mt-1">Call <code className="text-cyan-400">POST /capture-payment/:id</code> for full or partial capture.</div>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/5 p-4">
                <div className="text-xs uppercase tracking-wide text-slate-500">Refund</div>
                <div className="text-sm text-white mt-1">Call <code className="text-cyan-400">POST /refund-payment/:id</code> with optional <code className="text-cyan-400">amount</code> for partial refunds.</div>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/5 p-4 md:col-span-2">
                <div className="text-xs uppercase tracking-wide text-slate-500">Tokenize</div>
                <div className="text-sm text-white mt-1">
                  Tokenization happens via the SDK or the <code className="text-cyan-400">/card-collection-proxy</code> endpoint for server-side flows.
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4 mt-4">
              <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-4">
                <p className="text-sm text-amber-400">
                  Use for manual capture flows. Omit <code className="text-cyan-400">amount</code> for full capture.
                </p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-white mb-3">Request Body</h4>
            ...
            <div className="space-y-4 mt-4">
              <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-4">
                <p className="text-sm text-red-400">
                  Supports partial refunds by setting <code className="text-cyan-400">amount</code>.
                  Use <code className="text-cyan-400">Idempotency-Key</code> to safely retry.
                </p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-white mb-3">Request Body</h4>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-4 mt-4">
              <div className="p-4 rounded-lg bg-white/5">
                ...
                <p className="text-xs text-slate-500 mt-1">Fraud chargebacks become issuer's responsibility</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <Zap className="h-6 w-6 text-cyan-400 mb-2" />
                <h4 className="font-medium text-white text-sm">Frictionless Flow</h4>
                <p className="text-xs text-slate-500 mt-1">Low-risk transactions authenticate silently</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <Globe className="h-6 w-6 text-blue-400 mb-2" />
                <h4 className="font-medium text-white text-sm">PSD2 Compliant</h4>
                <p className="text-xs text-slate-500 mt-1">Required for European transactions</p>
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                    ...
                    <span className="text-xs text-slate-400 ml-2">Fully authenticated</span>
                  </div>
                  <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                    <code className="text-amber-400">A</code>
                    <span className="text-xs text-slate-400 ml-2">Attempted authentication</span>
                  </div>
                  <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20">
                    <code className="text-red-400">N</code>
                    <span className="text-xs text-slate-400 ml-2">Authentication failed</span>
                  </div>
                  <div className="p-3 rounded-lg bg-gray-500/10 border border-gray-500/20">
                    <code className="text-slate-400">U</code>
                    <span className="text-xs text-slate-400 ml-2">Unable to authenticate</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <EndpointCard method="POST" path="/threeds-authenticate/sessions/:id/challenge-complete" description="Complete challenge flow">
            <div className="mt-4">
              <p className="text-sm text-slate-400">
                ...
                The SDK handles this automatically when using <code className="text-cyan-400">authenticate3DS()</code>.
              </p>
            </div>
          </EndpointCard>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-4 gap-4 mt-4">
              <div className="p-4 rounded-lg bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/20 text-center">
                ...
                <div className="text-xs text-slate-400 mt-1">Higher Auth Rates</div>
              </div>
              <div className="p-4 rounded-lg bg-gradient-to-br from-[#19d1c3]/10 to-emerald-400/10 border border-cyan-400/20 text-center">
                <div className="text-2xl font-bold text-cyan-400">-3-5bps</div>
                <div className="text-xs text-slate-400 mt-1">Lower Interchange</div>
              </div>
              <div className="p-4 rounded-lg bg-gradient-to-br from-blue-500/10 to-cyan-500/10 border border-blue-500/20 text-center">
                ...
                <div className="text-xs text-slate-400 mt-1">Card Updates</div>
              </div>
              <div className="p-4 rounded-lg bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20 text-center">
                ...
                <div className="text-xs text-slate-400 mt-1">Less Fraud</div>
              </div>
            </div>
          </div>
          ...
          <EndpointCard method="GET" path="/network-tokens/:id" description="Get network token details">
            <div className="mt-4">
              <p className="text-sm text-slate-400">
                ...
                Include <code className="text-cyan-400">session_id</code> as a query parameter.
              </p>
            </div>
          </EndpointCard>
        ...
        <div className="space-y-6">
          ...
          <EndpointCard method="POST" path="/card-collection-proxy" description="Tokenize inbound card data and forward it to your endpoint" defaultExpanded>
            <div className="space-y-4 mt-4">
              ...
              <div>
                <h4 className="text-sm font-medium text-white mb-3">Response</h4>
                <p className="text-sm text-slate-400">Returns the downstream response from your destination URL after tokenization.</p>
              </div>
            </div>
          </EndpointCard>
            ...
            <div className="grid md:grid-cols-2 gap-4 mt-4">
              <div className="p-4 rounded-lg bg-white/5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Accept card data from booking engines, ERP systems, or partner platforms</p>
                </div>
              </div>
              <div className="p-4 rounded-lg bg-white/5 flex items-start gap-3">
                <Smartphone className="h-5 w-5 text-emerald-400 mt-0.5" />
                <div>
                  <h4 className="font-medium text-white text-sm">AI Agent Payments</h4>
                  <p className="text-xs text-slate-500 mt-1">Enable AI assistants to securely process payments on behalf of users</p>
                </div>
              </div>
              <div className="p-4 rounded-lg bg-white/5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Intercept webhooks containing card data and tokenize automatically</p>
                </div>
              </div>
              <div className="p-4 rounded-lg bg-white/5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Migrate from legacy payment systems without code changes</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-4 gap-4">
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400">Partner sends card data to your Atlas proxy URL</p>
              </div>
              <div className="text-center">
                <div className="h-10 w-10 rounded-full bg-[#19d1c3]/20 flex items-center justify-center mx-auto mb-2">
                  <span className="text-cyan-400 font-bold">2</span>
                </div>
                <p className="text-xs text-slate-400">Proxy extracts card data from configured field path</p>
              </div>
              <div className="text-center">
                <div className="h-10 w-10 rounded-full bg-[#19d1c3]/20 flex items-center justify-center mx-auto mb-2">
                  <span className="text-cyan-400 font-bold">3</span>
                </div>
                <p className="text-xs text-slate-400">Card data tokenized securely (never stored)</p>
              </div>
              <div className="text-center">
                <div className="h-10 w-10 rounded-full bg-[#19d1c3]/20 flex items-center justify-center mx-auto mb-2">
                  <span className="text-cyan-400 font-bold">4</span>
                </div>
                <p className="text-xs text-slate-400">Tokenized request forwarded to your endpoint</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-sm text-cyan-400">application/json</code>
                <p className="text-xs text-slate-500 mt-2">Standard JSON payloads with nested card objects</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-sm text-cyan-400">application/xml</code>
                <p className="text-xs text-slate-500 mt-2">XML payloads from legacy systems</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-sm text-cyan-400">application/x-www-form-urlencoded</code>
                <p className="text-xs text-slate-500 mt-2">Form data from web forms</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <h4 className="text-sm font-medium text-slate-400 mb-2">Incoming Request (from partner)</h4>
...

{
  ...
  "amount": 15000
              ...
              <div>
                <h4 className="text-sm font-medium text-slate-400 mb-2">Forwarded to Your Endpoint</h4>
            ...
            <div className="space-y-2">
              {[ { type: 'payment.authorized', desc: 'Payment authorized (manual capture mode)', color: 'blue' }, { type: 'payment.captured', desc: 'Payment captured successfully', color: 'green' }, { type: 'payment.failed', desc: 'Payment attempt failed', color: 'red' }, { type: 'payment.requires_action', desc: '3DS or other action required', color: 'amber' }, { type: 'payment.canceled', desc: 'Payment was canceled', color: 'gray' }, { type: 'refund.succeeded', desc: 'Refund processed successfully', color: 'green' }, { type: 'refund.failed', desc: 'Refund attempt failed', color: 'red' }, { type: 'dispute.created', desc: 'New dispute/chargeback opened', color: 'red' }, { type: 'dispute.updated', desc: 'Dispute status changed', color: 'amber' }, { type: 'dispute.closed', desc: 'Dispute resolved', color: 'gray' }, ].map((event) => (
                <div key={event.type} className="flex items-center gap-4 p-3 rounded-lg bg-white/5">
                  <code className="text-sm text-cyan-400 font-mono min-w-[200px]">{event.type}</code>
                  <span className="text-sm text-slate-400">{event.desc}</span>
                </div>
              ))}
            </div>
          </div>
...
function verifyWebhook(payload: string, signature: string, timestamp: string, secret: string): boolean {
            ...
            <div className="grid grid-cols-5 gap-2 mt-4">
              {['Immediately', '1 min', '5 min', '30 min', '2 hours'].map((time, i) => (
                <div key={i} className="p-3 rounded-lg bg-white/5 text-center">
                  <div className="text-xs text-slate-500">Attempt {i + 1}</div>
  ...
  resilience: {
    ...
    circuitBreakerThreshold: 5
              ...
              <div className="p-4 rounded-lg bg-white/5 space-y-3">
                <code className="text-sm text-cyan-400">Atlas.unmount()</code>
                <p className="text-sm text-slate-400">Clean up and remove the payment form. Call when navigating away or on component unmount.</p>
              </div>

              <div className="p-4 rounded-lg bg-white/5 space-y-3">
                <code className="text-sm text-cyan-400">Atlas.confirm(options?)</code>
                <p className="text-sm text-slate-400">Manually trigger payment confirmation. Usually automatic on form submit.</p>
  ...
  returnUrl: 'https://yoursite.com/complete' // For 3DS redirects
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-6">
            ...
            <div className="p-4 rounded-lg bg-white/5 space-y-3">
              <code className="text-sm text-cyan-400">Atlas.authenticate3DS(config)</code>
              <p className="text-sm text-slate-400">Initiate 3D Secure authentication for a payment.</p>
...
// result: { status: 'Y', authenticationValue: '...', eci: '05', liabilityShift: true }`}
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-6">
            ...
            <div className="p-4 rounded-lg bg-white/5 space-y-3">
              <code className="text-sm text-cyan-400">Atlas.createNetworkToken(config)</code>
              <p className="text-sm text-slate-400">Create a network token for improved authorization rates.</p>
...
// networkToken: { networkTokenId: 'nt_xxx', network: 'visa', cryptogram: '...' }`}
            ...
            <div className="p-4 rounded-lg bg-white/5 space-y-3">
              <code className="text-sm text-cyan-400">Atlas.getCryptogram(networkTokenId)</code>
              <p className="text-sm text-slate-400">Generate a new cryptogram for an existing network token.</p>
...
// cryptogram: { cryptogram: '...', cryptogramType: 'TAVV', expiresAt: '...' }`}
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-white/5 space-y-2">
                <code className="text-sm text-cyan-400">Atlas.isApplePayAvailable()</code>
                <p className="text-xs text-slate-400">Returns boolean - true if Apple Pay is available on this device/browser.</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5 space-y-2">
                <code className="text-sm text-cyan-400">Atlas.isGooglePayAvailable()</code>
                <p className="text-xs text-slate-400">Returns Promise&lt;boolean&gt; - checks Google Pay availability asynchronously.</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-white/5 space-y-2">
                <code className="text-sm text-cyan-400">Atlas.getCircuitBreakerState()</code>
                <p className="text-xs text-slate-400">Returns current state: 'CLOSED' | 'OPEN' | 'HALF_OPEN'</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5 space-y-2">
                <code className="text-sm text-cyan-400">Atlas.getHealthStatus()</code>
                <p className="text-xs text-slate-400">Returns Promise with endpoint health info</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5 space-y-2">
                <code className="text-sm text-cyan-400">Atlas.getPendingSyncTransactions()</code>
                <p className="text-xs text-slate-400">Returns transactions processed in fallback mode</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5 space-y-2">
                <code className="text-sm text-cyan-400">Atlas.syncPendingTransactions()</code>
                <p className="text-xs text-slate-400">Sync fallback transactions when primary recovers</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-6">
            ...
            <div className="p-4 rounded-lg bg-white/5 space-y-3">
              <p className="text-sm text-slate-400">Verify webhook signatures server-side using HMAC-SHA256.</p>
    ...
    "doc_url": "https://docs.atlas.co/errors/card_declined"
            ...
            <div className="space-y-2">
              {[ { code: '200', desc: 'Success', color: 'green' }, { code: '400', desc: 'Bad Request - Invalid parameters', color: 'amber' }, { code: '401', desc: 'Unauthorized - Invalid or missing API key', color: 'red' }, { code: '402', desc: 'Payment Required - Card declined', color: 'red' }, { code: '403', desc: 'Forbidden - Insufficient permissions', color: 'red' }, { code: '404', desc: 'Not Found - Resource doesn\'t exist', color: 'amber' }, { code: '409', desc: 'Conflict - Idempotency conflict', color: 'amber' }, { code: '429', desc: 'Too Many Requests - Rate limit exceeded', color: 'amber' }, { code: '500', desc: 'Server Error - Something went wrong', color: 'red' }, { code: '503', desc: 'Service Unavailable - Temporary outage', color: 'red' }, ].map((status) => (
                <div key={status.code} className="flex items-center gap-4 p-3 rounded-lg bg-white/5">
                  ...
                  <span className="text-sm text-slate-400">{status.desc}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-cyan-400">api_error</code>
                <p className="text-sm text-slate-400 mt-1">Server-side error. Safe to retry with exponential backoff.</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-cyan-400">authentication_error</code>
                <p className="text-sm text-slate-400 mt-1">Invalid API key or insufficient permissions. Check your credentials.</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-cyan-400">card_error</code>
                <p className="text-sm text-slate-400 mt-1">Card was declined. Show message to customer.</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-cyan-400">invalid_request_error</code>
                <p className="text-sm text-slate-400 mt-1">Invalid parameters. Check the 'param' field for details.</p>
              </div>
              <div className="p-4 rounded-lg bg-white/5">
                <code className="text-cyan-400">rate_limit_error</code>
                <p className="text-sm text-slate-400 mt-1">Too many requests. Implement backoff and retry.</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-2 gap-2">
              {[ { code: 'insufficient_funds', desc: 'Not enough funds' }, { code: 'lost_card', desc: 'Card reported lost' }, { code: 'stolen_card', desc: 'Card reported stolen' }, { code: 'expired_card', desc: 'Card has expired' }, { code: 'incorrect_cvc', desc: 'CVC code incorrect' }, { code: 'processing_error', desc: 'Temporary processor issue' }, { code: 'do_not_honor', desc: 'Issuer declined' }, { code: 'card_not_supported', desc: 'Card type not accepted' }, ].map((decline) => (
                <div key={decline.code} className="flex items-center gap-3 p-3 rounded-lg bg-white/5">
                  ...
                  <span className="text-xs text-slate-400">{decline.desc}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'testing' && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            ...
            <p className="text-sm text-slate-400">
              Use test API keys (starting with <code className="text-cyan-400">sk_test_</code>) to test your integration without processing real payments.
            </p>
            <div className="flex items-center gap-4 mt-4">
              <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                <span className="text-sm text-amber-400">Test Mode</span>
              </div>
              <span className="text-sm text-slate-500">No real charges • Test cards only • Simulated responses</span>
            </div>
          </div>

          <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
            <h3 className="font-semibold text-white">Test Card Numbers</h3>
            <p className="text-sm text-slate-400 mb-4">Use these card numbers with any future expiry date and any 3-digit CVC.</p>
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  {[ { number: '4242 4242 4242 4242', brand: 'Visa', desc: 'Succeeds' }, { number: '5555 5555 5555 4444', brand: 'Mastercard', desc: 'Succeeds' }, { number: '3782 822463 10005', brand: 'Amex', desc: 'Succeeds' }, { number: '6011 1111 1111 1117', brand: 'Discover', desc: 'Succeeds' }, ].map((card) => (
                    <div key={card.number} className="flex items-center gap-3 p-2 rounded bg-white/5">
                      ...
                      <span className="text-xs text-slate-500">{card.brand}</span>
                    </div>
                  ))}
                </div>
              </div>
...
curl_close($ch);`
      ...
      {activeTab === 'billing' && (
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/20 p-6">
            ...
            <p className="text-slate-400 text-sm">Build advanced billing logic with products, recurring prices, trials, usage-based billing, and automatic invoicing.</p>
          </div>
  ...
  -d '{
    ...
    "days_until_due": 30
          ...
          <EndpointCard method="POST" path="/invoices/:id/finalize" description="Finalize a draft invoice">
            <p className="text-sm text-slate-400">Transitions invoice from draft to open, making it ready for payment.</p>
  ...
  -H "Authorization: Bearer sk_test_xxx"`} />
          ...
          <EndpointCard method="POST" path="/invoices/:id/pay" description="Pay an invoice">
            <p className="text-sm text-slate-400">Attempt to collect payment on an open invoice.</p>
  ...
  -H "Authorization: Bearer sk_test_xxx"`} />
          ...
          <EndpointCard method="POST" path="/invoices/:id/void" description="Void an invoice">
            <p className="text-sm text-slate-400">Permanently void an invoice. Cannot be undone.</p>
      ...
      {activeTab === 'checkout' && (
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/20 p-6">
            ...
            <p className="text-slate-400 text-sm">Pre-built, conversion-optimized pages for checkout and customer management. No frontend code required.</p>
          </div>
  ...
  "expires_at": 1704931200
            ...
            <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
              ...
              <p className="text-sm text-slate-400">After creating a session, redirect the customer to the returned URL:</p>
...
window.location.href = url;`} />
  ...
  "expires_at": 1704848400
            ...
            <div className="rounded-xl dashboard-card border border-white/10 p-6 space-y-4">
              ...
              <p className="text-sm text-slate-400">Customers can manage their billing through the portal:</p>
