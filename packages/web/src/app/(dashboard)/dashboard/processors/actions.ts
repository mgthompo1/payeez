...
export async function createPspCredential(params: {
  ...
  if (userLookupError || !internalUser) {
    ...
    if (onboardError) {
      throw new Error(`Unable to find or create user account: ${onboardError.message}`)
    }
    ...
    if (!retryUser) {
      throw new Error('Unable to resolve user account after onboarding.')
    }
    userId = retryUser.id
  }
  ...
  if (membershipError || !membership) {
    throw new Error(`Unable to resolve tenant membership: ${membershipError?.message || 'no membership found'}`)
  }
