  ...
  return (
    <div className="p-8 space-y-6 max-w-screen-2xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="dashboard-heading text-2xl">Customers</h1>
          <p className="text-slate-500 mt-1">Manage your customer accounts</p>
        </div>
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogTrigger asChild>
            <Button className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:opacity-90 text-white font-medium shadow-lg shadow-cyan-500/20">
            ...
            <div className="space-y-4 mt-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className="bg-obsidian border-white/10 text-white min-h-[80px] placeholder:text-slate-500 focus:border-cyan-400 focus:ring-cyan-400/20"
                />
              </div>
      ...
      <div className="dashboard-card overflow-hidden">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-sm text-slate-500">{filteredCustomers.length} customers</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12">
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <p className="text-slate-400 mb-2">No customers yet</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10 bg-white/[0.02]">
                  ...
                  <th className="text-left py-3 px-6 text-xs font-medium text-slate-500 uppercase tracking-wider">Email</th>
                  <th className="text-left py-3 px-6 text-xs font-medium text-slate-500 uppercase tracking-wider">Subscriptions</th>
              ...
              <tbody>
                {filteredCustomers.map((customer) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <span className="text-slate-500 text-sm">{formatDate(customer.created_at)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Sheet open={!!selectedCustomer} onOpenChange={() => setSelectedCustomer(null)}>
        <SheetContent className="bg-charcoal border-white/10 w-[500px] sm:max-w-[500px] overflow-y-auto text-slate-200">
          <SheetHeader>
            ...
            <div className="mt-6 space-y-6">
              ...
              <div className="rounded-xl bg-white/5 border border-white/10 p-4">
                <div className="flex items-center gap-4 mb-4">
                  <div className="h-14 w-14 rounded-full bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center">
                    <span className="text-cyan-400 font-medium text-xl">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500">Customer since {formatDate(selectedCustomer.created_at)}</p>
                  </div>
                </div>
                {selectedCustomer.description && (
                  <p className="text-slate-400 text-sm">{selectedCustomer.description}</p>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                    ...
                    >
                      {copied === 'email' ? <Check className="h-4 w-4 text-cyan-400" /> : <Copy className="h-4 w-4" />}
                    </Button>
                  </div>
              ...
              {/* ID */}
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wider">Identifier</h3>
                <div className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                  <div>
                    <div className="text-xs text-slate-500">Customer ID</div>
                    <code className="text-sm text-cyan-400">{selectedCustomer.id}</code>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
              ...
              {selectedCustomer.billing_address && formatAddress(selectedCustomer.billing_address) && (
                <div className="space-y-3">
                  <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wider">Billing Address</h3>
              ...
              {/* Tax Status */}
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wider">Tax Status</h3>
              ...
              {/* Quick Actions */}
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wider">Quick Actions</h3>
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" className="border-white/10 text-slate-300 hover:bg-white/5 hover:text-cyan-400 hover:border-cyan-500/30">
              ...
              {selectedCustomer.metadata && Object.keys(selectedCustomer.metadata).length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wider">Metadata</h3>
                  <div className="p-3 rounded-lg bg-white/5">
