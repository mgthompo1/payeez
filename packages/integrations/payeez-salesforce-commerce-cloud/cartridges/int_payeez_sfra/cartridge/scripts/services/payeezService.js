...
/**
 * Payeez Service Module
 * Handles all API communication with Payeez payment orchestration
 */

var LocalServiceRegistry = require('dw/svc/LocalServiceRegistry');
var Logger = require('dw/system/Logger').getLogger('payeez', 'PayeezService');

var PAYEEZ_API_URL = 'https://yssswpqpwrrglgroxwok.supabase.co/functions/v1';

/**
 * Get Payeez HTTP Service
 * @returns {dw.svc.HTTPService} The configured HTTP service
 */
function getService() {
    return LocalServiceRegistry.createService('payeez.http', {
        createRequest: function (svc, args) {
            var Site = require('dw/system/Site');
            var secretKey = Site.current.getCustomPreferenceValue('payeezSecretKey');
...
function createSession(params) {
    ...
    service.setURL(PAYEEZ_API_URL + '/create-payment-session');
...
function confirmPayment(sessionId, paymentMethod) {
    var service = getService();
    service.setURL(PAYEEZ_API_URL + '/confirm-payment');
...
function getSession(sessionId) {
    var service = getService();
    service.setURL(PAYEEZ_API_URL + '/get-session?sessionId=' + sessionId);
...
function capture(transactionId, amount, currency) {
    var service = getService();
    service.setURL(PAYEEZ_API_URL + '/capture-payment');
...
function refund(transactionId, amount, currency, reason) {
    var service = getService();
    service.setURL(PAYEEZ_API_URL + '/refund-payment');
...
function chargeToken(token, amount, currency, reference) {
    var service = getService();
    service.setURL(PAYEEZ_API_URL + '/charge-token');
