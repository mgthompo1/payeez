 ...
 * Processes async webhook notifications from Payeez
 */

var Logger = require('dw/system/Logger').getLogger('payeez', 'PayeezWebhook');
...
function verifySignature(payload, signature) {
    ...
    var webhookSecret = Site.current.getCustomPreferenceValue('payeezWebhookSecret');
...
function handlePaymentCompleted(data) {
    ...
    if (order) {
        Transaction.wrap(function () {
            order.custom.payeezPaymentStatus = 'completed';
            order.custom.payeezTransactionId = data.transactionId;
...
function handlePaymentFailed(data) {
    ...
    if (order) {
        Transaction.wrap(function () {
            order.custom.payeezPaymentStatus = 'failed';
            order.custom.payeezErrorMessage = data.errorMessage || 'Payment declined';
...
function handlePaymentCaptured(data) {
    ...
    if (order) {
        Transaction.wrap(function () {
            order.custom.payeezCaptureId = data.captureId;
            order.custom.payeezCapturedAmount = data.amount;
...
function handlePaymentRefunded(data) {
    ...
    if (order) {
        Transaction.wrap(function () {
            order.custom.payeezRefundId = data.refundId;
            order.custom.payeezRefundedAmount = (order.custom.payeezRefundedAmount || 0) + data.amount;

            // Check if fully refunded
            if (order.custom.payeezRefundedAmount >= order.totalGrossPrice.value) {
                order.custom.payeezPaymentStatus = 'refunded';
            } else {
                order.custom.payeezPaymentStatus = 'partially_refunded';
            }
        });
...
function handlePaymentVoided(data) {
    ...
    if (order) {
        Transaction.wrap(function () {
            order.custom.payeezPaymentStatus = 'voided';
 ...
 * @param {string} sessionId - Payeez session ID
 ...
 * @param {string} transactionId - Payeez transaction ID
 * @returns {dw.order.Order|null} Order or null
 */
