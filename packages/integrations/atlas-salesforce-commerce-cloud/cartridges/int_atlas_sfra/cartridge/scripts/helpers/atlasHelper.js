...
/**
 * Atlas Helper Module
 * Utility functions for Atlas integration
 */
...
function buildSessionParams(basket) {
    ...
    return {
        ...
        merchantId: Site.current.getCustomPreferenceValue('atlasMerchantId'),
        customer: {
        ...
        returnUrl: require('dw/web/URLUtils').abs('Atlas-ThreeDSReturn').toString(),
        metadata: {
            platform: 'SFCC',
 ...
 * @param {string} customerNo - Customer number
 * @param {Object} cardData - Card data from Atlas
...
function saveCard(customerNo, cardData) {
    ...
    try {
        Transaction.wrap(function () {
            var cardObj = CustomObjectMgr.createCustomObject('AtlasSavedCard', UUIDUtils.createUUID());
...
function getConfig() {
    return {
        publicKey: Site.current.getCustomPreferenceValue('atlasPublicKey'),
        isTestMode: Site.current.getCustomPreferenceValue('atlasTestMode'),
        applePayEnabled: Site.current.getCustomPreferenceValue('atlasApplePayEnabled'),
        googlePayEnabled: Site.current.getCustomPreferenceValue('atlasGooglePayEnabled'),
        merchantId: Site.current.getCustomPreferenceValue('atlasMerchantId')
    };
}
