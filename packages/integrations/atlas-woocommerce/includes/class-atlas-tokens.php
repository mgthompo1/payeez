...
class Atlas_Tokens {
    ...
    public static function get_token( $token_id, $user_id ) {
        ...
        if ( ! $token || $token->get_user_id() !== $user_id || 'atlas' !== $token->get_gateway_id() ) {
            return null;
        }
     ...
     *
     * @param int   $user_id   User ID.
     * @param array $card_data Card data from Atlas.
    ...
    public static function save_card( $user_id, $card_data, $is_default = false ) {
        ...
        $existing_tokens = WC_Payment_Tokens::get_customer_tokens( $user_id, 'atlas' );
