 ...
 * Payeez API Client
 *
 * Handles communication with the Payeez payment orchestration API.
 *
...
class Payeez_API {
    ...
    public function __construct( $public_key, $secret_key, $test_mode = true, $debug_mode = false ) {
        ...
        // Payeez API URL (Supabase Edge Functions)
    ...
    private function request( $method, $endpoint, $body = array() ) {
        ...
        if ( $status_code >= 400 ) {
            $error_message = isset( $data['error'] ) ? $data['error'] : __( 'API request failed.', 'payeez-woocommerce' );
            return new WP_Error( 'payeez_api_error', $error_message );
        }
    ...
    private function log( $message ) {
        ...
        if ( function_exists( 'wc_get_logger' ) ) {
            ...
            $logger->debug( $message, array( 'source' => 'payeez' ) );
        }
    }
    ...
    private function log_error( $message ) {
        if ( function_exists( 'wc_get_logger' ) ) {
            $logger = wc_get_logger();
            $logger->error( $message, array( 'source' => 'payeez' ) );
        }
    }
}
