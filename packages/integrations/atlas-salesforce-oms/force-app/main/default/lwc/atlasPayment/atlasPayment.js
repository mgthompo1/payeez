...
import ATLAS_SDK from '@salesforce/resourceUrl/AtlasSDK';

import createSession from '@salesforce/apex/AtlasPaymentController.createSession';
import confirmPayment from '@salesforce/apex/AtlasPaymentController.confirmPayment';
import getSavedCards from '@salesforce/apex/AtlasPaymentController.getSavedCards';
import getConfig from '@salesforce/apex/AtlasPaymentController.getConfig';

export default class AtlasPayment extends LightningElement {
    @api recordId;
    @api objectApiName;
    ...
    async connectedCallback() {
        try {
            ...
            // Load Atlas SDK
            await loadScript(this, ATLAS_SDK);

            // Initialize Atlas
            this.atlas = window.Atlas(this.config.publicKey, {
    ...
    async handlePayment() {
        ...
        try {
            ...
            if (this.selectedCard !== 'new') {
                // Use saved card
                paymentResult = await this.atlas.confirmPayment(sessionResult.sessionId, {
            ...
            if (paymentResult.requiresAction) {
                // Handle 3DS
                paymentResult = await this.atlas.handleNextAction(paymentResult);
                if (paymentResult.error) {
                    throw new Error(paymentResult.error.message);
                }
            }
